...
public:
  ...
  void logFrontendPoseKitti34();
  void handleVIO();
  void handleLIO();
  void savePCD();
  void processImu();
  void detectElevatorScene();
  ...
  bool isElevatorScene(const PointCloudXYZI::Ptr &cloud, float &length, float &width, float &height);
  ...
  PointCloudXYZI::Ptr feats_for_color_; ///< copy of LIO body cloud cached for the subsequent VIO coloring step
  PointCloudXYZI::Ptr pcl_w_wait_pub;
  PointCloudXYZI::Ptr pcl_wait_pub;
  PointCloudXYZRGB::Ptr pcl_wait_save;
  PointCloudXYZI::Ptr pcl_wait_save_intensity;

  ofstream fout_pre, fout_out, fout_visual_pos, fout_lidar_pos, fout_points, fout_frontend_kitti34, fout_elevator_scene_flag;
  uint64_t frontend_pose_id_ = 0;
  uint64_t elevator_scene_id_ = 0;
  ...
  ros::Publisher pubKfGravity;
  ros::Publisher pubElevatorScene;
  ...
  bool is_elevator_scene_ = false;
  float elevator_length_ = 0.0f;
  float elevator_width_ = 0.0f;
  float elevator_height_ = 0.0f;
};
#endif