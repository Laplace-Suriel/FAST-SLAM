...
namespace
{
constexpr int kMatOutColWidth = 16;
constexpr int kMatOutFeatWidth = 10;

void writeMatOutHeader(std::ostream &os)
{
  os << std::left
     << std::setw(kMatOutColWidth) << "time"
     << std::setw(kMatOutColWidth) << "pos_x"
     << std::setw(kMatOutColWidth) << "pos_y"
     << std::setw(kMatOutColWidth) << "pos_z"
     << std::setw(kMatOutColWidth) << "vel_x"
     << std::setw(kMatOutColWidth) << "vel_y"
     << std::setw(kMatOutColWidth) << "vel_z"
     << std::setw(kMatOutColWidth) << "bg_x"
     << std::setw(kMatOutColWidth) << "bg_y"
     << std::setw(kMatOutColWidth) << "bg_z"
     << std::setw(kMatOutColWidth) << "ba_x"
     << std::setw(kMatOutColWidth) << "ba_y"
     << std::setw(kMatOutColWidth) << "ba_z"
     << std::setw(kMatOutColWidth) << "inv_expo_time"
     << std::setw(kMatOutColWidth) << "reserved_1"
     << std::setw(kMatOutColWidth) << "reserved_2"
     << std::setw(kMatOutFeatWidth) << "feat_num"
     << '\n'
     << std::right;
}

void writeMatOutRow(std::ostream &os,
                    double time,
                    const V3D &pos,
                    const V3D &vel,
                    const V3D &bias_g,
...
{
  os << std::right << std::scientific << std::setprecision(9)
     << std::setw(kMatOutColWidth) << time
     << std::setw(kMatOutColWidth) << pos(0)
     << std::setw(kMatOutColWidth) << pos(1)
     << std::setw(kMatOutColWidth) << pos(2)
     << std::setw(kMatOutColWidth) << vel(0)
     << std::setw(kMatOutColWidth) << vel(1)
     << std::setw(kMatOutColWidth) << vel(2)
     << std::setw(kMatOutColWidth) << bias_g(0)
     << std::setw(kMatOutColWidth) << bias_g(1)
     << std::setw(kMatOutColWidth) << bias_g(2)
     << std::setw(kMatOutColWidth) << bias_a(0)
     << std::setw(kMatOutColWidth) << bias_a(1)
     << std::setw(kMatOutColWidth) << bias_a(2)
     << std::setw(kMatOutColWidth) << inv_expo_time
     << std::setw(kMatOutColWidth) << 0.0
     << std::setw(kMatOutColWidth) << 0.0
     << std::setw(kMatOutFeatWidth) << feature_count
     << '\n';
}

V3D replaceVerticalComponentOnGravityAxis(const V3D &pos_updated, const V3D &pos_reference, const V3D &gravity)
{
  ...
  const V3D g_hat = gravity / g_norm;
  const V3D vertical_ref = g_hat * pos_reference.dot(g_hat);
  const V3D horizontal_updated = pos_updated - g_hat * pos_updated.dot(g_hat);
  return horizontal_updated + vertical_ref;
}
}

bool LIVMapper::isElevatorScene(const PointCloudXYZI::Ptr &cloud, float &length, float &width, float &height)
{
  ...
  float min_p[3] = {std::numeric_limits<float>::max(), std::numeric_limits<float>::max(), std::numeric_limits<float>::max()};
  float max_p[3] = {std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest(), std::numeric_limits<float>::lowest()};

  for (const auto &pt : cloud->points)
  {
    Eigen::Vector3f p(pt.x, pt.y, pt.z);
    p -= centroid;
    Eigen::Vector3f proj = axes.transpose() * p;
    for (int i = 0; i < 3; ++i)
    {
      min_p[i] = std::min(min_p[i], proj[i]);
      max_p[i] = std::max(max_p[i], proj[i]);
    }
  }

  float extents[3] = {max_p[0] - min_p[0], max_p[1] - min_p[1], max_p[2] - min_p[2]};
  std::sort(extents, extents + 3);
  width = extents[0];
  length = extents[1];
  height = extents[2];

  const float long_side = std::max(length, width);
  const float short_side = std::min(length, width);

  const float ELEVATOR_MIN_SIDE = 0.8f;
  const float ELEVATOR_MAX_SIDE = 3.5f;
  const float ELEVATOR_MAX_ASPECT = 2.5f;
  const float ELEVATOR_MIN_HEIGHT = 1.5f;
  const float ELEVATOR_MAX_HEIGHT = 3.5f;
  ...
  const float aspect = long_side / short_side;

  const bool size_ok = (length >= ELEVATOR_MIN_SIDE && length <= ELEVATOR_MAX_SIDE &&
                        width >= ELEVATOR_MIN_SIDE && width <= ELEVATOR_MAX_SIDE);
  const bool shape_ok = (aspect <= ELEVATOR_MAX_ASPECT);
  const bool height_ok = (height >= ELEVATOR_MIN_HEIGHT && height <= ELEVATOR_MAX_HEIGHT);

  return size_ok && shape_ok && height_ok;
}
      ...
      extR(M3D::Identity())
{
  ...
  feats_for_color_.reset(new PointCloudXYZI());
  ...
  frame_publish_stamp_ = ros::Time::now();
...
void LIVMapper::initializeFiles() 
{
  ...
  fout_elevator_scene_flag.open(DEBUG_FILE_DIR("elevator_scene_flag.txt"), std::ios::out);
  writeMatOutHeader(fout_out);
  if (fout_elevator_scene_flag.is_open())
  {
    fout_elevator_scene_flag << "id is_elevator_scene length width height" << std::endl;
  }
  fout_frontend_kitti34.open(std::string(ROOT_DIR) + "Log/result/frontend_pose_kitti_3x4.txt", std::ios::out);
}
...
void LIVMapper::initializeSubscribersAndPublishers(ros::NodeHandle &nh, image_transport::ImageTransport &it) 
{
  ...
  pubKfGravity = nh.advertise<geometry_msgs::Vector3Stamped>("/kf_gravity", 100);
  pubElevatorScene = nh.advertise<std_msgs::Bool>("/lio/elevator_scene", 100);
...
void LIVMapper::logFrontendPoseKitti34()
{
  if (!fout_frontend_kitti34.is_open())
  {
    return;
  }

  const M3D &R = _state.rot_end;
  const V3D &t = _state.pos_end;
  fout_frontend_kitti34 << std::fixed << std::setprecision(9)
...
void LIVMapper::handleVIO() 
{
  ...
  const V3D gravity_vec = _state.gravity;
  const double gravity_norm = gravity_vec.norm();
  const M3D G_R_I0 = (gravity_norm < 1e-6)
                      ? M3D::Identity()
                      : Quaterniond::FromTwoVectors(gravity_vec, V3D(0, 0, -1)).toRotationMatrix();
  const V3D pos_end_gravity = G_R_I0 * _state.pos_end;
  const V3D vel_end_gravity = G_R_I0 * _state.vel_end;
  ...
  fout_pre << std::setw(20) << LidarMeasures.last_lio_update_time - _first_lidar_time << " "
            << pos_end_gravity.transpose() << " " << vel_end_gravity.transpose() << " " << _state.bias_g.transpose() << " "
            << _state.bias_a.transpose() << " " << V3D(_state.inv_expo_time, 0, 0).transpose() << std::endl;
  ...
  if (is_elevator_scene_)
  {
    _state.pos_end = replaceVerticalComponentOnGravityAxis(_state.pos_end, state_propagat.pos_end, _state.gravity);
    _state.vel_end = replaceVerticalComponentOnGravityAxis(_state.vel_end, state_propagat.vel_end, _state.gravity);
  }
  ...
  // Publish body-frame cloud with all original points.
  // Default color is black for points not valid for RGB projection.
  PointCloudXYZI::Ptr laserCloudFullRes = feats_for_color_;
  if (laserCloudFullRes && !laserCloudFullRes->empty())
  {
    const bool canColorize = (vio_manager && !vio_manager->img_rgb.empty() &&
                              vio_manager->new_frame_ && vio_manager->new_frame_->cam_);

    const int body_size = laserCloudFullRes->points.size();
    PointCloudXYZRGB::Ptr laserCloudBodyRGB(new PointCloudXYZRGB());
    laserCloudBodyRGB->reserve(body_size);

    cv::Mat img_rgb;
    if (canColorize) {
      img_rgb = vio_manager->img_rgb;
    }

    for (int i = 0; i < body_size; i++)
    {
      PointTypeRGB pointRGB;
      pointRGB.x = laserCloudFullRes->points[i].x;
      pointRGB.y = laserCloudFullRes->points[i].y;
      pointRGB.z = laserCloudFullRes->points[i].z;
      // Default color for filtered points: dark gray
      pointRGB.r = 128;
      pointRGB.g = 128;
      pointRGB.b = 128;
      ...
      if (canColorize) {
        ...
        V3D p_w(pt_world.x, pt_world.y, pt_world.z);
        V3D pf(vio_manager->new_frame_->w2f(p_w));
        if (pf[2] >= 0 && pf.norm() > blind_rgb_points) {
      ...
      laserCloudBodyRGB->push_back(pointRGB);
    }

    sensor_msgs::PointCloud2 laserCloudBodyMsg;
    pcl::toROSMsg(*laserCloudBodyRGB, laserCloudBodyMsg);
    laserCloudBodyMsg.header.stamp = frame_publish_stamp_;
    laserCloudBodyMsg.header.frame_id = "aft_mapped";
    pubLaserCloudBody.publish(laserCloudBodyMsg);

    if (!canColorize) {
      if (!vio_manager)
        ROS_WARN_THROTTLE(1.0, "[handleVIO] Publish body cloud without RGB: vio_manager is null.");
      else if (vio_manager->img_rgb.empty())
        ROS_WARN_THROTTLE(1.0, "[handleVIO] Publish body cloud without RGB: img_rgb is empty.");
      else if (!vio_manager->new_frame_)
        ROS_WARN_THROTTLE(1.0, "[handleVIO] Publish body cloud without RGB: new_frame_ is null.");
      else if (!vio_manager->new_frame_->cam_)
        ROS_WARN_THROTTLE(1.0, "[handleVIO] Publish body cloud without RGB: camera model (cam_) is null.");
    }
  }
  else
  {
    ROS_WARN_THROTTLE(1.0, "[handleVIO] Skip body cloud publish: point cloud is null or empty.");
  }
  ...
  euler_cur = RotMtoEuler(_state.rot_end);
  const V3D gravity_vec_out = _state.gravity;
  const double gravity_norm_out = gravity_vec_out.norm();
  const M3D G_R_I0_out = (gravity_norm_out < 1e-6)
                          ? M3D::Identity()
                          : Quaterniond::FromTwoVectors(gravity_vec_out, V3D(0, 0, -1)).toRotationMatrix();
  const V3D pos_end_gravity_out = G_R_I0_out * _state.pos_end;
  const V3D vel_end_gravity_out = G_R_I0_out * _state.vel_end;
  /* fout_out << std::setw(20) << LidarMeasures.last_lio_update_time - _first_lidar_time << " " << euler_cur.transpose() * 57.3 << " "
            << _state.pos_end.transpose() << " " << _state.vel_end.transpose() << " " << _state.bias_g.transpose() << " "
            << _state.bias_a.transpose() << " " << V3D(_state.inv_expo_time, 0, 0).transpose() << " " << feats_undistort->points.size() << std::endl; */

  writeMatOutRow(fout_out,
...
{    
  ...
  if (ba_bg_est_en)
  {
    ...
    {
      p_imu->enable_bias_est();
    }
  }
  if (fout_elevator_scene_flag.is_open())
  {
    fout_elevator_scene_flag << elevator_scene_id_++ << " " << (is_elevator_scene_ ? 1 : 0) << " "
                             << std::fixed << std::setprecision(4)
                             << elevator_length_ << " " << elevator_width_ << " " << elevator_height_ << std::endl;
  }
  ...
  if (is_elevator_scene_)
  {
    ...
    _state.vel_end = replaceVerticalComponentOnGravityAxis(_state.vel_end, state_propagat.vel_end, _state.gravity);
    voxelmap_manager->state_ = _state;
    voxelmap_manager->RebuildResidualListForCurrentState();
  }
  ...
  std_msgs::Bool elevator_scene_msg;
  elevator_scene_msg.data = is_elevator_scene_;
  pubElevatorScene.publish(elevator_scene_msg);
  // Log only when LIO/LO odometry is actually published.
  logFrontendPoseKitti34();
  ...
  // Cache a deep copy for the subsequent VIO coloring step.
  *feats_for_color_ = *laserCloudFullRes;
...
/*   printf("\033[1;34m+-------------------------------------------------------------+\033[0m\n");
  ...
  gravity_msg.header.stamp = frame_publish_stamp_;
  gravity_msg.header.frame_id = "camera_init";
  gravity_msg.vector.x = _state.gravity(0);
  gravity_msg.vector.y = _state.gravity(1);
  gravity_msg.vector.z = _state.gravity(2);
  pubKfGravity.publish(gravity_msg);
}
...
void LIVMapper::detectElevatorScene()
{
  is_elevator_scene_ = false;
  elevator_length_ = 0.0f;
  elevator_width_ = 0.0f;
  elevator_height_ = 0.0f;
  if (LidarMeasures.lio_vio_flg != LIO)
  {
    return;
  }

  PointCloudXYZI::Ptr cloud = feats_undistort;
  const char *tag = "LIO";
  ...
  is_elevator_scene_ = isElevatorScene(cloud, elevator_length_, elevator_width_, elevator_height_);
  if (is_elevator_scene_)
  {
    const double bag_time = LidarMeasures.last_lio_update_time - _first_lidar_time;
    ROS_WARN_STREAM("[ " << tag << " ] Elevator scene detected. t=" << bag_time
                    << " L=" << elevator_length_ << " W=" << elevator_width_ << " H=" << elevator_height_);
  }
}
...
void LIVMapper::publish_img_rgb(const image_transport::Publisher &pubImage, VIOManagerPtr vio_manager)
{
  ...
  out_msg.header.stamp = frame_publish_stamp_;
...
void LIVMapper::publish_frame_world(const ros::Publisher &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  laserCloudmsg.header.stamp = frame_publish_stamp_; //.fromSec(last_timestamp_lidar);
...
void LIVMapper::publish_visual_sub_map(const ros::Publisher &pubSubVisualMap)
{
  ...
  *sub_pcl_visual_map_pub = *laserCloudFullRes;
  if (1)
  {
    ...
    laserCloudmsg.header.stamp = frame_publish_stamp_;
    laserCloudmsg.header.frame_id = "camera_init";
...
void LIVMapper::publish_effect_world(const ros::Publisher &pubLaserCloudEffect, const std::vector<PointToPlane> &ptpl_list)
{
  ...
  laserCloudFullRes3.header.stamp = frame_publish_stamp_;
...
void LIVMapper::publish_odometry(const ros::Publisher &pubOdomAftMapped)
{
  ...
  odomAftMapped.header.stamp = frame_publish_stamp_; //.ros::Time()fromSec(last_timestamp_lidar);
...
{
  msg_body_pose.header.stamp = frame_publish_stamp_;
